...
### Solution
```{r}
print("History repeats itself, first as tragedy, second as farce.")
```
:::
...
### Solution 2
```{r}
print(paste0("I love the number ", 1 + 2 + 3 + 4 + 5, "!"))
```
...
width <- 3.5 # This is a comment! Comments are not executed.
height <- 2.5  
width * height  # Note the typo in height
```
...
price <- 100
discount_factor <- 0.1
final_price <- price * (1 - discount_factor) 
```
...
### Solution
`NA` encodes an *unknown* value. It's not known if `NA` equals `2`, hence the resulting value is *also* unknown!
```{r}
NA == 2
```
...
abs(0.3 - (0.1+0.2))
```
We can check if this number is small, e.g., less than `1e-7`. 
