...
### Solution
```{r}
matrix(seq(9), ncol= 3, byrow = TRUE)
```
...
### Solution
A simple way to verify near equality of two matrices is as follows:
...
```{r}
svd_ <- svd(x)
svd_$v %*% diag(1/svd_$d) %*% t(svd_$u) %*% y
```
...
slow_means_2 <- \(x) {
  means <- rep(0, nrow(x))
  for (i in seq(nrow(x))) means[i] <- sum(x[i,]) / nrow(x)
  means  
}
...
```{r}
microbenchmark::microbenchmark(rowMeans(x), slow_means(x), slow_means_2(x))
```
:::
